...
class Resource:
	def __init__(desired_temp: int):
		...
		self.curr_temp = None
		self.last_temp = None
		self.fanStatus = 0 			# -1 – decrease fan speed, 1 – increase, 0 – maintain

	def ordersForFan() -> int:
		if self.status == 0:
			if curr_temp > on_temp: self.fanStatus = 1
		elif curr_temp < off_temp: self.fanStatus = 0
...
class Disk(Resource):
	...
	def getTemp() -> int:
		res = os.popen("sudo smartctl -d sat --all /dev/%s | grep Temperature_Celsius | awk '{print $10}'" % self.name).readline()
...
class Fan:
	...
	def calcrpm(channel):
		...
		try:
			...
			self.rpm = (30 / (t1-t0)) # Since there are two flanks per rotation we need to half it
...
gpio.add_event_detect(FAN_SPEED_GPIO, gpio.RISING, callback = fan.calcrpm)
...
if __name__ == '__main__':
	...
	while killer.thread_dont_terminate:	# main loop
		...
		fanOrders = set()
		for r in resources:
			r.getTemp()		# get cpu and disk temperature
			fanOrders.add(r.ordersForFan())

		if 1 in fanOrders: fan.increase_rpm()
		elif -1 in fanOrders: fan.decrease_rpm()
		else: fan.desired_rpm = 0
